...
std::shared_ptr<CActor> CHaroldFactory::Create()
{
    ...
    auto headb = make_shared<CImageDrawable>(L"Head Bottom", L"images/indiansantaheadtop.png");
    headb->SetCenter(Point(200, 200));
    headb->SetPosition(Point(-30, -200));
    shirt->AddChild(headb);

    auto headt = make_shared<CHeadTop>(L"Head Top", L"images/indianheadtopsanta.png");
    headt->SetCenter(Point(180, 140));
