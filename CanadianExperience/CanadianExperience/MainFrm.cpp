...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWndEx)
	...
	ON_COMMAND(ID_MACHINE_SWITCH32793, &CMainFrame::OnMachineSwitch32793)
	ON_COMMAND(ID_MACHINE_MACHINE3, &CMainFrame::OnMachineMachine3)
	ON_COMMAND(ID_MACHINE_SETMACHINE1TIME, &CMainFrame::OnMachineSetmachine1time)
	ON_COMMAND(ID_MACHINE_SETMACHINE2TIME, &CMainFrame::OnMachineSetmachine2time)
END_MESSAGE_MAP()
...
void CMainFrame::OnMachineSwitch32793()
{
	auto machine = mPicture->GetMachine();
...
void CMainFrame::OnMachineMachine3()
{
	auto machine2 = mPicture->GetMachine2();
	CMachineDlg dlg(machine2);
	if (dlg.DoModal() == IDOK)
	{
		machine2->SetMachineFrame(0);
		machine2->SetFrameRate(30);
	}
}


/**
 * OnMachineSetMachinemainframe
 */
void CMainFrame::OnMachineSetmachine1time()
{
	auto machineAdapter = mPicture->GetMachineAdapter().get();
	CMachineTimeDlg dlg;
	dlg.SetAdapter(machineAdapter);
	if (dlg.DoModal() == IDOK)
	{
		dlg.Take();
	}
 ...
 void CMainFrame::OnMachineSetmachine2time()
 {
	 auto machineAdapter = mPicture->GetMachineAdapter2().get();
	 CMachineTimeDlg dlg;
	 dlg.SetAdapter(machineAdapter);
	 if (dlg.DoModal() == IDOK)
	 {
