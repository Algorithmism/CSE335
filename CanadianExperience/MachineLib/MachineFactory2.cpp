...
/**
 * \brief Create Machine Factory 2
 * \returns 
 */
std::shared_ptr<CWorkingMachine> CMachineFactory2::CreateMachine()
{
	...
	// This is how I make a connection from a source to a sink..0
	...
	for (int i = 0; i < 5; i++)
	{
		...
		// A cam
		auto cam = make_shared<CCam>(30, 16);
		cam->SetImage(L"images/snowflake.png", true);
		cam->SetPhase(-0.25);
		...
		roller->SetImage(L"images/snow.png");
		...
		case 3:
			...
			channel = machine->GetWavPlayer()->CreateChannel(L"audio/bell-d1.wav");
			cam->AddPin(12);
			break;

		case 4:
			bar->SetImage(L"images/bar-cyan.png");
			channel = machine->GetWavPlayer()->CreateChannel(L"audio/bell-f1.wav");
			cam->AddPin(14);
			cam->AddPin(15);
			cam->AddPin(16);
			cam->AddPin(17);
	...
	auto song = machine->GetWavPlayer()->CreateChannel(L"audio/indianjingbells.wav");
	song->Play();
	return machine;
}
