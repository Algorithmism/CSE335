...
namespace Testing
{
	...
	public:
		...
		TEST_METHOD(TestCAquariumHitTest)
		{
			...
			Assert::IsTrue(aquarium.HitTest(100, 200) == fish2, L"Testing fish 2 at 100, 200");
			Assert::IsFalse(aquarium.HitTest(100, 200) == fish1, L"Testing fish 1 at 100, 200");
		...
		void PopulateAllTypes(CAquarium *aquarium) 
		{
			...
			shared_ptr<CFishKarp> fish5 = make_shared<CFishKarp>(aquarium);
			fish5->SetLocation(200, 200);
			aquarium->Add(fish5);
		...
		void TestAllTypes(wstring filename) 
		{
			...
			Assert::IsTrue(regex_search(xml,
				wregex(L"<item x=\"100\" y=\"200\" speedX=\"37\\.0754.*\" speedY=\"25\\.4814.*\"")));
			Assert::IsTrue(regex_search(xml, 
				wregex(L"<item x=\"400\" y=\"400\" speedX=\"31\\.2540.*\" speedY=\"44\\.6958.*\"")));
			Assert::IsTrue(regex_search(xml, 
				wregex(L"<item x=\"600\" y=\"100\" speedX=\"5\\.9160.*\" speedY=\"11\\.1438.*\"")));
			Assert::IsTrue(regex_search(xml, 
				wregex(L"<item x=\"300\" y=\"300\"")));
			Assert::IsTrue(regex_search(xml, 
				wregex(L"<item x=\"200\" y=\"200\" speedX=\"23\\.1955.*\" speedY=\"45\\.6205.*\"")));

			//Ensure the types are correct
			Assert::IsTrue(regex_search(xml,
				wregex(L"<aqua><item.* type=\"beta\"/><item.* type=\"dory\"/><item.* type=\"nemo\"/><item.* type=\"castle\"/><item.* type=\"karp\"/></aqua>")));
		...
		*/
		...
		void TestThreeBetas(wstring filename)
		{
			...
			// Ensure the positions are correct
			Assert::IsTrue(regex_search(xml, wregex(L"<item x=\"100\" y=\"200\" speedX=\"37\\.0754.*\" speedY=\"25\\.4814.*\"")));
			Assert::IsTrue(regex_search(xml, wregex(L"<item x=\"400\" y=\"400\" speedX=\"31\\.2540.*\" speedY=\"44\\.6958.*\"")));
			Assert::IsTrue(regex_search(xml, wregex(L"<item x=\"600\" y=\"100\" speedX=\"5\\.9160.*\" speedY=\"11\\.1438.*\"")));
