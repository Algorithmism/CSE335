...
*/
CFishKarp::CFishKarp(CAquarium *aquarium) : CItem(aquarium)
{
	mFishImage = unique_ptr<Bitmap>(Bitmap::FromFile(FishKarpImageName.c_str()));
	if (mFishImage->GetLastStatus() != Ok)
	{
		wstring msg(L"Failed to open ");
		msg += FishKarpImageName;
 ...
 */
void CFishKarp::Draw(Gdiplus::Graphics *graphics)
{
...
bool CFishKarp::HitTest(int x, int y)
{
	double wid = mFishImage->GetWidth();
	double hit = mFishImage->GetHeight();
